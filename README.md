...
- **Built for AI agents (parallel track)**: a separate workstream makes spliti drivable by an AI agent through a closed *write → run → observe → correct* loop. `plugin/time`'s `Manual` virtual clock + a seeded `plugin/rng` make a run **deterministic**; the `inspect` package dumps the live ECS world to **JSON** (generically, no codegen — entity-reference fields included); the `check` package runs a game headlessly for N frames with scripted input and captures that JSON (+ an optional PNG). `spliti check -ticks 120 -out world.json` ties it together — same code + seed → byte-identical state to diff across edits. `AGENTS.md` and `spliti manifest` give an agent the engine and a project's surface as context, and `spliti mcp` exposes the whole loop as MCP tools. See the **AI track** in [docs/roadmap.md](docs/roadmap.md).

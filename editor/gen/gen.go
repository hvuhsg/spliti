...
// CheckDir is where the generated headless-verification target lives.
func (p *Project) CheckDir() string { return filepath.Join(p.Root, ".spliti", "check") }

// GenerateCheck writes .spliti/check/{go.mod,main.go}: a headless target that
// runs the game's default scene and systems through the check harness with a
// deterministic clock and seeded RNG, then dumps the world as JSON (and an
// optional frame PNG). Unlike the editor target it needs no component registry
// — package inspect dumps every component generically.
func (p *Project) GenerateCheck() error {
	dir := p.CheckDir()
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return err
	}
	if err := p.render(checkGomodTmpl, filepath.Join(dir, "go.mod")); err != nil {
		return err
	}
	return p.render(checkMainTmpl, filepath.Join(dir, "main.go"))
}
...
var checkGomodTmpl = template.Must(template.New("checkgomod").Parse(`module spliti-check-target
...
var checkMainTmpl = template.Must(template.New("checkmain").Parse(`// Code generated by spliti gen; DO NOT EDIT.
//go:build !js

// Command spliti-check-target runs the game's default scene and systems
// headlessly for a fixed number of deterministic frames, then writes the world
// state as JSON (and optionally a frame PNG). It is what "spliti check" builds
// and runs — the agent's verify step.
package main
...
{{- end}}
	...
	a.AddSystems(schedule.Startup, app.Chain(
		...
		app.System(scenes.{{.DefaultSceneFunc}}),
	))
	...
	fmt.Printf("check: ran %d frames, %d entities → %s\n", res.Frames, len(res.Dump.Entities), *out)
}
`))

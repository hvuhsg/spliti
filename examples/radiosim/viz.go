...
// receiver's propagation through the engine seam, animates the wavefront shells,
// recolors the coverage heatmap, draws the contributing rays, and refreshes the
// HUD.
func vizSystem(c *app.Ctx) {
	scene := app.GetResource[Scene](c)
	live := app.GetResource[Live](c)
	...
	if scene == nil || live == nil || view == nil || eng == nil {
		return
	}
	if tm != nil {
	...
	app.Query2[render3d.Transform3D, receiverTag](c, func(_ ecs.Entity, t *render3d.Transform3D, _ *receiverTag) {
		live.Rx.Pos = t.Translation
	})

	// Run the propagation for the active receiver via the engine seam.
	live.Paths = eng.Paths(scene.Tx, live.Rx, scene.Sim, scene.Cfg)
	live.PowerW, live.Taps = sim.Received(live.Paths)
	...
	updateHUD(c, scene, live)
	updateUI(c, scene, eng)
}
...
func drawRays(c *app.Ctx, live *Live) {
	if len(live.Paths) == 0 {
		return
	}
	maxAmp := 0.0

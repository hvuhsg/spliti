...
func newScene() *Scene {
	...
	faces = append(faces, sim.NewFace(
	...
	for _, side := range []float32{-1, 1} {
		...
		for x < streetHalfX-4 {
			...
			faces = append(faces, sim.Box(min, max)...)
			x += w + gap
		}
	}
	...
	cfg.Transmission = true // let the signal penetrate walls (attenuated)
	cfg.Diffraction = true  // bend around corners so shadows aren't dead zones
	cfg.NumRays = 12000     // SBR ray budget (used when the SBR engine is active)

	scene := &Scene{
		Sim:       sim.NewScene(faces),
		Buildings: buildings,
		Tx:        tx,
		Cfg:       cfg,
		Grid:      grid,
		RxHeight:  rxHeight,
		WallMat:   sim.MatConcrete,
		recompute: true,
	}
	// Face 0 is the ground; paint it (it stays ground) and leave the rest as the
	// default wall material — buildings are repainted live via the M key.
	scene.Sim.FaceMat[0] = sim.MatDryGround
	return scene
}
...
func setup(c *app.Ctx) {
	...
	render3d.SpawnDirectionalLight(cmd, render3d.DirectionalLight{
		Direction: m.Vec3{X: -0.3, Y: -1, Z: -0.2},

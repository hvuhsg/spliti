...
// --- two-ray ground reflection: closed-form check, via the Engine seam ---
...
func TestTwoRayGroundReflection(t *testing.T) {
	...
	rx := Rx{Pos: m.Vec3{X: d, Y: hr}}

	// Large ground face in the y=0 plane spanning the geometry, normal +Y. The
	// ground is a perfect conductor (Γ = -1), so the reflection coefficient comes
	// from the material, not from cfg.Reflection — which we set to 0 here to prove
	// the material drives it.
	ground := []Face{NewFace(m.Vec3{X: -10, Z: -10}, m.Vec3{X: 100}, m.Vec3{Z: 20})}
	sc := NewScene(ground)
	sc.FaceMat[0] = MatMetal
	cfg := Config{MaxOrder: 1, Reflection: 0}

	var eng Engine = ImageEngine{}
	paths := eng.Paths(tx, rx, sc, cfg)
	if len(paths) != 2 {
...
// --- coverage: shadow behind a building, via the Engine seam ---
...
func TestCoverageShadow(t *testing.T) {
	...
	var eng Engine = ImageEngine{}
	field := eng.Coverage(tx, grid, sc, Config{MaxOrder: 1, Reflection: -0.7})

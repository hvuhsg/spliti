...
// many line-of-sight queries the coverage grid and the engines issue. A nil *BVH
// is valid: LineOfSight falls back to a brute-force scan, which is fine for a
// handful of buildings. Build one when the query count is large.
type BVH struct {
	faces []Face
	nodes []bvhNode
